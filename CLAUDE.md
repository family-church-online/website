...
- **Related sermons** — `src/data/related-sermons.json` maps each sermon slug to a short list of related sermon slugs, computed via Jaccard similarity on the canonical tag set. `src/data/sermon-tags.json` is a precomputed `slug → topic tags[]` map used by the Worker to recompute related sermons without fetching all MDX files. Both files are rebuilt by `scripts/build-sermon-tags.mjs` (called by `pnpm sermons:import`) and by `SermonPublishWorkflow` on every publish. The individual sermon page imports `related-sermons.json` statically at build time — no runtime computation.
...
- **`import.meta.glob` on sermon MDX must use `import: 'frontmatter'`** — sermon MDX files contain full transcripts (~10 KB each × 200+ files). Any eager glob without `import: 'frontmatter'` bundles all transcript bodies into the Worker. Always use `import.meta.glob<FrontmatterType>('...sermons/*.mdx', { eager: true, import: 'frontmatter' })`. The returned values are the frontmatter objects directly, not `{ frontmatter: ... }` wrappers.
- **Timeline block address** — `Timeline.astro` calls `getConfig()` at render time and formats `config.seo.address` into a single display line. There is no per-block `locationName` field. The `mapsUrl` field on the block adds a "Get directions" link. A `Church` JSON-LD schema is emitted automatically when an address exists.
- **Team block carousel** — `Team.astro` uses CSS `scroll-snap` for native swipe on mobile. Prev/next arrows and dot indicators are driven by vanilla JS scoped to a random `uid` per instance. The carousel activates only when `members.length > 1`; a single member renders as a plain card.
- **Groups page filters** — client-side `<select>` dropdowns (day, time of day, area) plus an "Open only" checkbox. A "Clear filters" button appears only when any filter is active.
- **`SITE_URL` must be a build-time env var** — `getSiteUrl()` in `astro.config.mjs` reads `process.env.SITE_URL` during the Astro build. In Cloudflare Workers, the "Runtime variables and secrets" in the dashboard are NOT available at build time. Set `SITE_URL` under the **build** environment variables (Workers & Pages → Settings → Builds) for the sitemap and canonical URLs to use the correct domain.
